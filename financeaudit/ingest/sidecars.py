        ...
        for tgt, (i, typ) in idx.items():
            ...
            try:
                if typ == "date":
                    ...
                    val, raw_a, canon = parse_german_amount(v)
                    rec[tgt] = val
                    rec[tgt + "_raw"] = raw_a
                    rec[tgt + "_dec"] = canon
                elif typ == "int":
...
def parse_xlsx_files(data_dir: Path, registry, manifest):
    ...
    manifest.add(file=rel, file_hash=fh, size_bytes=path.stat().st_size,
                 ...
                 population_scope="permission matrix of the 9 posting-capable users per 31.12.2025 (SoD basis)")
    ...
    manifest.add(file=rel, file_hash=fh, size_bytes=path.stat().st_size,
                 ...
                 population_scope="trial balance FY2025 (43 accounts) + net-income reconciliation sheet")
    ...
    manifest.add(file=rel, file_hash=fh, size_bytes=path.stat().st_size,
                 ...
                 population_scope="management-prepared subledger/GL reconciliation per 31.12.2025 (zero differences claimed)")
...
def parse_doc_units(data_dir: Path, registry, manifest):
    ...
    # pdfs: one 'page' registry unit per page + one 'para' doc_unit per text block
    pdf_scopes = {
